%{?scl:%scl_package perl-Devel-Caller}

Name:           %{?scl_prefix}perl-Devel-Caller
Version:        2.06
Release:        21%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(PadWalker) >= 0.08
BuildRequires:  %{?scl_prefix}perl(XSLoader)
# Tests:
BuildRequires:  %{?scl_prefix}perl(Test::More)
BuildRequires:  %{?scl_prefix}perl(vars)
Requires:       %{?scl_prefix}perl(:MODULE_COMPAT_%(%{?scl:scl enable %{scl} '}eval "$(perl -V:version)";echo $version%{?scl:'}))
Requires:       %{?scl_prefix}perl(PadWalker) >= 0.08

%global __provides_exclude %{?__provides_exclude:__provides_exclude|}^%{?scl_prefix}perl\\(DB\\)
%global __requires_exclude %{?__requires_exclude:%__requires_exclude|}^%{?scl_prefix}perl\\(PadWalker\\)$
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 2.06-21
- SCL
